...
/// <summary>Covers <see cref="SystemCacheProvider.ToBytes"/>: scaling the reported page count by the system
/// page size, and rejecting the "not reported" zeros and an overflowing product. The
...
public class SystemCacheProviderTests {
    [Theory]
    ...
    public void ToBytes_PlausibleReading_ScalesByThePageSize(ulong pages, ulong pageSize, ulong expected) {
        Assert.Equal(expected, SystemCacheProvider.ToBytes(pages, pageSize));
    }
    ...
    public void ToBytes_ZeroInput_ReturnsNull(ulong pages, ulong pageSize) {
        Assert.Null(SystemCacheProvider.ToBytes(pages, pageSize));
    }
    ...
    public void ToBytes_OverflowingProduct_ReturnsNull() {
        Assert.Null(SystemCacheProvider.ToBytes(ulong.MaxValue, 4096));
    }
    ...
    public void ToBytes_ProductAtTheLimit_StillReturnsAValue() {
        Assert.Equal(ulong.MaxValue - 1, SystemCacheProvider.ToBytes((ulong.MaxValue - 1) / 2, 2));
    }
}
