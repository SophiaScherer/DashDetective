...
/// <summary>
/// Simulates dichromatic vision (Viénot, Brettel and Mollon 1999), so a color-blind-safe palette can be
...
internal static class ColorVisionSimulator {
    /// <summary>How far apart two colors must stay under simulation, against a 2.3 ΔE just-noticeable
    /// difference. 20 is the light theme's measured ceiling — the best five colors clearing 3:1 on white
    /// separate by ~21.5, where dark reaches 35-50 — so raising it means finding better colors.</summary>
    internal const double MinimumSeparation = 20.0;

    /// <summary>A color as a dichromat of the given kind would see it.</summary>
    ...
    /// <summary>CIE76 color difference in Lab, which is close enough to perceptual for a "are these two
    ...
    /// <summary>The two colors' separation once both are seen through the deficiency.</summary>
    ...
    private static (double L, double A, double B) Lab(Color c) {
        ...
        // sRGB to XYZ (D65), then normalized by the white point.
