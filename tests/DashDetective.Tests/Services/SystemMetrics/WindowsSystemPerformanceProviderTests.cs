...
/// <summary>Covers <see cref="WindowsSystemPerformanceProvider.ToBytes"/>: scaling the reported page count by the
...
public class WindowsSystemPerformanceProviderTests {
    [Theory]
    ...
    public void ToBytes_PlausibleReading_ScalesByThePageSize(ulong pages, ulong pageSize, ulong expected) {
        Assert.Equal(expected, WindowsSystemPerformanceProvider.ToBytes(pages, pageSize));
    }
    ...
    public void ToBytes_ZeroInput_ReturnsNull(ulong pages, ulong pageSize) {
        Assert.Null(WindowsSystemPerformanceProvider.ToBytes(pages, pageSize));
    }
    ...
    public void ToBytes_OverflowingProduct_ReturnsNull() {
        Assert.Null(WindowsSystemPerformanceProvider.ToBytes(ulong.MaxValue, 4096));
    }
    ...
    public void ToBytes_ProductAtTheLimit_StillReturnsAValue() {
        Assert.Equal(ulong.MaxValue - 1, WindowsSystemPerformanceProvider.ToBytes((ulong.MaxValue - 1) / 2, 2));
    }
}
