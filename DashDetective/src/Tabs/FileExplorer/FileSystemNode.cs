...
public partial class FileSystemNode : ObservableObject {
    ...
    private async Task LoadChildrenAsync() {
        ...
        foreach (var s in subs)
            Children.Add(new FileSystemNode(s.Name, s.FullPath, true, s.HasChildren, _includeHidden, _onSelected));
    }

    // A childless marker row shown until the real children are enumerated on expand.
    private static FileSystemNode LoadingPlaceholder() => new("Loading…", "", false, false, static () => false);
}
